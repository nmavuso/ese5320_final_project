...
typedef struct {
    ...
    XLzw_fpga_uio_map maps[ MAX_UIO_MAPS ];
} XLzw_fpga_uio_info;

/***************** Variable Definitions **************************************/
static XLzw_fpga_uio_info uio_info;
...
static int uio_info_read_name(XLzw_fpga_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLzw_fpga_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLzw_fpga_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLzw_fpga_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLzw_fpga_Initialize(XLzw_fpga *InstancePtr, const char* InstanceName) {
	XLzw_fpga_uio_info *InfoPtr = &uio_info;
