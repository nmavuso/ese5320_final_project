...
void XLzw_fpga_Set_s(XLzw_fpga *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XLzw_fpga_WriteReg(InstancePtr->Control_BaseAddress, XLZW_FPGA_CONTROL_ADDR_S_DATA, (u32)(Data));
    XLzw_fpga_WriteReg(InstancePtr->Control_BaseAddress, XLZW_FPGA_CONTROL_ADDR_S_DATA + 4, (u32)(Data >> 32));
}

u64 XLzw_fpga_Get_s(XLzw_fpga *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XLzw_fpga_ReadReg(InstancePtr->Control_BaseAddress, XLZW_FPGA_CONTROL_ADDR_S_DATA);
    Data += (u64)XLzw_fpga_ReadReg(InstancePtr->Control_BaseAddress, XLZW_FPGA_CONTROL_ADDR_S_DATA + 4) << 32;
    return Data;
}
