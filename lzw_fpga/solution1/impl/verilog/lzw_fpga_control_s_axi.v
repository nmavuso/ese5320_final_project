...
localparam
    ...
    ADDR_S_DATA_1           = 6'h14,
    ADDR_S_CTRL             = 6'h18,
    ADDR_OUTPUT_CODE_DATA_0 = 6'h1c,
    ADDR_OUTPUT_CODE_DATA_1 = 6'h20,
    ADDR_OUTPUT_CODE_CTRL   = 6'h24,
    ADDR_OUTPUT_SIZE_DATA_0 = 6'h28,
    ADDR_OUTPUT_SIZE_DATA_1 = 6'h2c,
    ADDR_OUTPUT_SIZE_CTRL   = 6'h30,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_OUTPUT_CODE_DATA_0: begin
                    rdata <= int_output_code[31:0];
                end
                ADDR_OUTPUT_CODE_DATA_1: begin
                    rdata <= int_output_code[63:32];
                end
...
// int_output_code[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_code[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_CODE_DATA_0)
            int_output_code[31:0] <= (WDATA[31:0] & wmask) | (int_output_code[31:0] & ~wmask);
    end
end

// int_output_code[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_code[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_CODE_DATA_1)
            int_output_code[63:32] <= (WDATA[31:0] & wmask) | (int_output_code[63:32] & ~wmask);
    end
end
...
// int_output_size[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_size[63:32] <= 0;
    else if (ACLK_EN) begin
