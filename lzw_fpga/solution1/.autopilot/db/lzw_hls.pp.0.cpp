# 1 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 375 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.cpp" 2
...
# 2 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.cpp" 2
# 1 "/mnt/pollux/software/xilinx/2020.2/Vitis_HLS/2020.2/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/limits.h" 1 3
# 37 "/mnt/pollux/software/xilinx/2020.2/Vitis_HLS/2020.2/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/limits.h" 3
...
# 38 "/mnt/pollux/software/xilinx/2020.2/Vitis_HLS/2020.2/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/limits.h" 2 3
# 3 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.cpp" 2
# 1 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.h" 1
...
# 5 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.h" 2
...
__attribute__((sdx_kernel("lzw_fpga", 0))) void lzw_fpga(const char *s, int *output_code, int *output_size, char *output);
...
# 4 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.cpp" 2


void string_copy(char *dest, const char *src, int max_len) {
    int i = 0;
    VITIS_LOOP_8_1: for (i = 0; i < max_len - 1 && src[i] != '\0'; i++) {
        dest[i] = src[i];
    }
    dest[i] = '\0';
}
...
int string_compare(const char *s1, const char *s2) {
    int i = 0;
    VITIS_LOOP_16_1: while (s1[i] != '\0' && s2[i] != '\0') {
        if (s1[i] != s2[i]) {
            return s1[i] - s2[i];
        }
        i++;
...
int string_length(const char *str) {
    int len = 0;
    VITIS_LOOP_27_1: while (str[len] != '\0' && len < 1024) {
        len++;
    }
    return len;
}
...
void encoding(const char *input, int *output_code, int *output_size) {
    ...
    VITIS_LOOP_40_1: for (int i = 0; i <= 255; i++) {
    ...
    VITIS_LOOP_49_2: for (int i = 1; input[i] != '\0'; i++) {
        char c = input[i];
        char temp[1024];
        int p_len = string_length(p);


        string_copy(temp, p, 1024);
        temp[p_len] = c;
        temp[p_len + 1] = '\0';


        int found = -1;
        VITIS_LOOP_61_3: for (int j = 0; j < table_size; j++) {
        ...
        if (found != -1) {
            ...
            string_copy(p, temp, 1024);
        } else {

            VITIS_LOOP_73_4: for (int j = 0; j < table_size; j++) {
            ...
            if (table_size < 4096) {
                string_copy(table[table_size].str, temp, 1024);
    ...
    VITIS_LOOP_94_5: for (int j = 0; j < table_size; j++) {
        if (string_compare(p, table[j].str) == 0) {
            output_code[out_index++] = table[j].code;
            break;
        }
...
void decoding(const int *encoded_data, int encoded_size, char *output) {
    ...
    VITIS_LOOP_111_1: for (int i = 0; i <= 255; i++) {
        table[i].str[0] = (char)i;
        table[i].str[1] = '\0';
        table[i].code = i;
    }
    ...
    if (current_code >= 0 && current_code < table_size) {
        string_copy(temp, table[current_code].str, 1024);
        string_copy(output + out_index, temp, 1024 - out_index);
    ...
    VITIS_LOOP_131_2: for (int i = 1; i < encoded_size; i++) {
        ...
        if (out_index + string_length(new_str) >= 1024 - 1) {
            return;
        }

        string_copy(output + out_index, new_str, 1024 - out_index);
...
# 169 "../../../../../../home1/n/nmavuso/ese5320_final_project/hls/lzw_hls.cpp"
